...
// Librerías para estabilidad (Brownout y Registro)
#include "soc/soc.h"
...
// CORRECCIÓN: Se han intercambiado los pines para corregir la inversión de giro (Izquierda/Derecha)
// NOTA IMPORTANTE SOBRE CONFLICTOS:
// - GPIO 1 (TX): Usado por Motor Derecho. Interferirá con el Monitor Serie si se activa.
// - GPIO 16: Usado por Motor Derecho. CONFLICTO CON PSRAM.
//   DEBE DESACTIVARSE PSRAM EN EL IDE DE ARDUINO (Tools -> PSRAM -> Disabled).
std::vector<MOTOR_PINS> motorPins =
{
  {2, 1, 16},  // Motor derecho (Antes 12, 13)
  {2, 12, 13}, // Motor izquierdo (Antes 1, 16)
};
...
{
  // Serial.printf("Valor recibido: %d\n", inputValue); // Comentado para evitar jitter si hay motor en GPIO 1
...
void setupCamera()
{
  ...
  // Configuración de imagen optimizada para estabilidad
  config.xclk_freq_hz = 10000000; // Bajado a 10MHz para reducir calor y ruido
  config.pixel_format = PIXFORMAT_JPEG;
  config.frame_size = FRAMESIZE_VGA;
  config.jpeg_quality = 12;
  config.fb_count = 1; // Reducido a 1 para ahorrar RAM y evitar fallos
...
void setup()
{
  // Desactivar detector de brownout (evita reinicios por caídas de tensión)
  WRITE_PERI_REG(RTC_CNTL_BROWN_OUT_REG, 0);

  // Inicializar comunicación serie para depuración (AVISO: Conflictivo con GPIO 1)
  // Serial.begin(115200);
  // Serial.println("\n--- Robot Explorador FLEXI PETS Iniciando ---");

  // Primero la cámara para detectar errores de hardware temprano
  setupCamera();
  ...
  // Desactivar sleep de WiFi para mejorar latencia de video
  WiFi.setSleep(false);

  // Configurar rutas del servidor y WebSockets
